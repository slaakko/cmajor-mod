...
module cmajor.binary.message.schema.ast;

import cmajor.binary.message.protocol;

namespace cmajor::binary::message::schema::ast {
...
std::string ClassNode::FullCmajorClassName() const
{
    std::string fullNsName;
    Node* parent = Parent();
    while (parent)
...
uint32_t ClassNode::MessageId() const
{
    return cmajor::bmp::MakeMessageId(cmajor::bmp::GetHashCode(FullClassName()));
}

std::string ClassNode::MessageIdConstantName() const
{
    std::string messageIdConstant = "bmp" + Id() + "Id";
...
} // cmajor::namespace binary::message::schema::ast
