...
#ifndef CM_EMITTER_INCLUDED
#define CM_EMITTER_INCLUDED
#include <cmllvm/api.hpp>
#include <cmllvm/emitting_context.hpp>
#include <util_inc/uuid.hpp>
#include <util_inc/source_pos.hpp>
#include <vector>

namespace cmllvm {

struct CMLLVM_API Pad
{
...
class CMLLVM_API EmittingDelegate
{
public:
    ...
    virtual std::string GetSourceFilePath(const soul::ast::SourcePos& sourcePos, const util_inc::uuid& moduleId) { return std::string(); }
    virtual Pad* CurrentPad() { return nullptr; }
...
public:
    ...
    virtual cmllvm::EmittingContext* EmittingContext() const = 0;
    ...
    virtual void* GetIrTypeByTypeId(const util_inc::uuid& typeId) = 0;
    virtual void SetIrTypeByTypeId(const util_inc::uuid& typeId, void* irType) = 0;
    ...
    virtual void* CreateDITypeForEnumType(const std::string& name, const std::string& mangledName, const soul::ast::SourcePos& sourcePos, const util_inc::uuid& moduleId, const std::vector<void*>& enumConstantElements,
        uint64_t sizeInBits, uint32_t alignInBits, void* underlyingDIType) = 0;
    virtual void* CreateIrDIForwardDeclaration(void* irType, const std::string& name, const std::string& mangledName, const soul::ast::SourcePos& sourcePos, const util_inc::uuid& moduleId) = 0;
    virtual uint64_t GetOffsetInBits(void* classIrType, int layoutIndex) = 0;
    virtual void* CreateDITypeForClassType(void* irType, const std::vector<void*>& memberVariableElements, const soul::ast::SourcePos& classSourcePos, const util_inc::uuid& moduleId, const std::string& name, void* vtableHolderClass,
        const std::string& mangledName, void* baseClassDIType) = 0;
    virtual void MapFwdDeclaration(void* fwdDeclaration, const util_inc::uuid& typeId) = 0;
    virtual void* GetDITypeByTypeId(const util_inc::uuid& typeId) const = 0;
    virtual void SetDITypeByTypeId(const util_inc::uuid& typeId, void* diType, const std::string& typeName) = 0;
    virtual void* GetDIMemberType(const std::pair<util_inc::uuid, int32_t>& memberVariableId) = 0;
    virtual void SetDIMemberType(const std::pair<util_inc::uuid, int32_t>& memberVariableId, void* diType) = 0;
    virtual void* CreateDIMemberType(void* scope, const std::string& name, const soul::ast::SourcePos& sourcePos, const util_inc::uuid& moduleId, uint64_t sizeInBits, uint64_t alignInBits, uint64_t offsetInBits, void* diType) = 0;
    ...
    virtual void MapClassPtr(const util_inc::uuid& typeId, void* classPtr, const std::string& className) = 0;
    ...
    virtual void* GetDebugInfoForFile(const soul::ast::SourcePos& sourcePos, const util_inc::uuid& moduleId) = 0;
    ...
    virtual void SetFunction(void* function_, int32_t fileIndex, const util_inc::uuid& sourceModuleId, const util_inc::uuid& functionId) = 0;
    ...
    virtual void AddLocalVariable(const std::string& localVariableName, const util_inc::uuid& typeId, void* irObject) = 0;
    ...
    virtual void* CreateDIMethod(const std::string& name, const std::string& mangledName, const soul::ast::SourcePos& sourcePos, const util_inc::uuid& moduleId, void* subroutineType, unsigned virtuality, unsigned vtableIndex, void* vtableHolder,
        unsigned flags) = 0;
    virtual void* CreateDIFunction(const std::string& name, const std::string& mangledName, const soul::ast::SourcePos& sourcePos, const util_inc::uuid& moduleId, void* subroutineType, unsigned flags) = 0;
    virtual void SetDISubprogram(void* function, void* subprogram) = 0;
    virtual void* CreateAlloca(void* irType) = 0;
    virtual void* CreateDIParameterVariable(const std::string& name, int index, const soul::ast::SourcePos& sourcePos, const util_inc::uuid& moduleId, void* irType, void* allocaInst) = 0;
    virtual void* CreateDIAutoVariable(const std::string& name, const soul::ast::SourcePos& sourcePos, const util_inc::uuid& moduleId, void* irType, void* allocaInst) = 0;
    ...
    virtual void* CreateLexicalBlock(const soul::ast::SourcePos& sourcePos, const util_inc::uuid& moduleId) = 0;
