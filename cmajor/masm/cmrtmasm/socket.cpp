...
{
    errorId = 0;
    if (cmajor::masm::rt::initError != 0)
    {
        errorId = cmajor::masm::rt::initError;
        return nullptr;
    }
    SOCKET s = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (s == INVALID_SOCKET)
    {
        int errorCode = cmajor::masm::rt::GetLastSocketError();
        std::string errorMessage = cmajor::masm::rt::GetSocketErrorMessage(errorCode);
        errorId = cmajor::masm::rt::AllocateError(errorMessage);
        return nullptr;
    }
    return new cmajor::masm::rt::SocketData(s);
}
...
void RtmBindSocket(void* socketHandle, int32_t port, int32_t& errorId)
{
    ...
    if (!socketHandle)
    {
        errorId = cmajor::masm::rt::AllocateError("invalid socket handle");
        return;
    }
    cmajor::masm::rt::SocketData* socketData = static_cast<cmajor::masm::rt::SocketData*>(socketHandle);
...
void* RtmAcceptSocket(void* socketHandle, int32_t& errorId)
{
    ...
    void* acceptedSocketHandle = new cmajor::masm::rt::SocketData(a);
