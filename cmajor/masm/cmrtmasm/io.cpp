...
void* RtmOpenFile(const char* filePath, cmajor::masm::rt::OpenMode openMode, int32_t& errorId)
{
    return cmajor::masm::rt::OpenFile(filePath, openMode, errorId);
}

void* RtmOpenStdFile(int handle, int32_t& errorId)
{
    return cmajor::masm::rt::OpenStdFile(handle, errorId);
}
...
{
    return cmajor::masm::rt::CloseFile(fileHandle, errorId);
}

bool RtmDisposeFile(void* fileHandle, int32_t& errorId)
{
    return cmajor::masm::rt::DisposeFile(fileHandle, errorId);
}

int64_t RtmWrite(void* fileHandle, const uint8_t* buffer, int64_t count, int32_t& errorId)
{
    return cmajor::masm::rt::WriteFile(fileHandle, buffer, count, errorId);
}

bool RtmWriteByte(void* fileHandle, uint8_t x, int32_t& errorId)
{
    return cmajor::masm::rt::WriteByte(fileHandle, x, errorId);
}
...
{
    return cmajor::masm::rt::Eof(fileHandle);
}

bool RtmGetFileError(void* fileHandle, int32_t& errorId)
{
    return cmajor::masm::rt::GetFileError(fileHandle, errorId);
}

bool RtmSeek(void* fileHandle, int64_t pos, cmajor::masm::rt::Origin origin, int32_t& errorId)
{
    return cmajor::masm::rt::SeekFile(fileHandle, pos, origin, errorId);
}

int64_t RtmTell(void* fileHandle, int32_t& errorId)
{
    return cmajor::masm::rt::TellFile(fileHandle, errorId);
}
...
bool RtmLastWriteTimeLess(const char* filePath1, const char* filePath2, int& errorId)
{
    ...
    if (ec)
    {
        errorId = cmajor::masm::rt::AllocateError("could not get last write time of file '" + std::string(filePath1) + "': " + util::PlatformStringToUtf8(ec.message()));
        return false;
    }
    std::filesystem::file_time_type t2 = std::filesystem::last_write_time(f2, ec);
    if (ec)
    {
        errorId = cmajor::masm::rt::AllocateError("could not get last write time of file '" + std::string(filePath2) + "': " + util::PlatformStringToUtf8(ec.message()));
        return false;
    }
    if (t1 < t2)
    {
...
int64_t RtmGetFileSize(const char* filePath, int32_t& errorId)
{
    ...
    if (ec)
    {
        errorId = cmajor::masm::rt::AllocateError("could not get size of file '" + std::string(filePath) + "': " + util::PlatformStringToUtf8(ec.message()));
        return -1;
    }
    return fileSize;
}
...
bool RtmRemoveFile(const char* filePath, int32_t& errorId)
{
    ...
    if (ec)
    {
        errorId = cmajor::masm::rt::AllocateError("could not remove file '" + std::string(filePath) + "': " + util::PlatformStringToUtf8(ec.message()));
        return false;
    }
    return true;
}
...
bool RtmCopyFile(const char* sourceFilePath, const char* targetFilePath, int32_t& errorId)
{
    ...
    if (ec)
    {
        errorId = cmajor::masm::rt::AllocateError("could not copy file '" + std::string(sourceFilePath) + "': " + util::PlatformStringToUtf8(ec.message()));
        return false;
    }
    return true;
}
...
bool RtmMoveFile(const char* sourceFilePath, const char* targetFilePath, int32_t& errorId)
{
    ...
    if (ec)
    {
        errorId = cmajor::masm::rt::AllocateError("could not move file '" + std::string(sourceFilePath) + "': " + util::PlatformStringToUtf8(ec.message()));
        return false;
    }
    return true;
}
