...
module cmdoclib.diagram;

import soul.lexer;

namespace cmdoclib {

void GenerateDiagramDrawingCode(util::CodeFormatter& formatter, const std::string& svgId)
{
...
DiagramNode::DiagramNode(int index_, int level_, const std::string& name_, bool subject_, bool hasDerivedNodes_, const std::string& link_) :
    index(index_), level(level_), name(name_), subject(subject_), hasDerivedNodes(hasDerivedNodes_), link(link_)
{
}

void DiagramNode::Write(util::CodeFormatter& formatter)
{
    formatter.Write("{ ");
...
void GenerateDiagramCode(std::vector<DiagramNode>& diagramNodes, const std::string& drawFunctionName, const std::string& svgId, const std::string& scriptFilePath)
{
    ...
    util::CodeFormatter formatter(scriptFile);
