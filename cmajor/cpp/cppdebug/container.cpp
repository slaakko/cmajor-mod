...
std::unique_ptr<util::JsonValue> Pointer::Value()
{
...
std::unique_ptr<util::JsonValue> Pointer::Deref()
{
    if (hasAddress && address == 0)
    {
...
uint64_t Pointer::Address()
{
    ...
    std::unique_ptr<util::JsonValue> value = Value();
    if (value->Type() == util::JsonValueType::object)
    {
        util::JsonObject* jsonObject = static_cast<util::JsonObject*>(value.get());
        util::JsonValue* successValue = jsonObject->GetField(U"success");
        bool success = false;
        if (successValue && successValue->Type() == util::JsonValueType::boolean)
        {
            success = static_cast<util::JsonBool*>(successValue)->Value();
        }
        if (success)
        {
            util::JsonValue* addrValue = jsonObject->GetField(U"value");
            if (addrValue && addrValue->Type() == util::JsonValueType::string)
            {
                util::JsonString* addrStr = static_cast<util::JsonString*>(addrValue);
                std::string addressStr = util::ToUtf8(addrStr->Value());
...
std::unique_ptr<util::JsonValue> Pointer::ValueMember(const std::string& name)
{
    std::unique_ptr<Pointer> memberPtr(new Pointer(container, "(*" + expression + ")." + name));
...
uint64_t GetContainerAddress(Debugger& debugger, ContainerClassTemplateKind containerKind, const std::string& containerVarExpr)
{
    ...
    if (!addressExpr.empty())
    {
        ...
        util::JsonValue* resultValue = debugger.ReleaseResult();
        if (resultValue && resultValue->Type() == util::JsonValueType::object)
        {
            ...
            util::JsonValue* successValue = resultObject->GetField(U"success");
            bool success = false;
            if (successValue && successValue->Type() == util::JsonValueType::boolean)
            {
                util::JsonBool* successBool = static_cast<util::JsonBool*>(successValue);
                success = successBool->Value();
            }
            if (success)
            {
                util::JsonValue* addressValue = resultObject->GetField(U"value");
                if (addressValue && addressValue->Type() == util::JsonValueType::string)
                {
                    util::JsonString* addrStr = static_cast<util::JsonString*>(addressValue);
                    std::string addressStr = util::ToUtf8(addrStr->Value());
                    address = ParseHex(addressStr);
                }
            }
...
std::unique_ptr<util::JsonValue> ForwardListIterator::Current()
{
...
std::unique_ptr<util::JsonValue> LinkedListIterator::Current()
{
    return nodePtr->ValueMember("value");
}
...
std::unique_ptr<util::JsonValue> HashtableIterator::Current()
{
...
std::unique_ptr<util::JsonValue> TreeIterator::Current()
{
    Container& container = GetContainer();
...
std::unique_ptr<util::JsonValue> ListIterator::Current()
{
...
int64_t Container::Count(const std::string& expr)
{
    ...
    std::unique_ptr<util::JsonValue> resultValue(debugger.ReleaseResult());
    if (resultValue && resultValue->Type() == util::JsonValueType::object)
    {
        util::JsonObject* resultObject = static_cast<util::JsonObject*>(resultValue.get());
        util::JsonValue* successValue = resultObject->GetField(U"success");
        bool success = false;
        if (successValue && successValue->Type() == util::JsonValueType::boolean)
        {
        ...
        if (success)
        {
            util::JsonValue* countValue = resultObject->GetField(U"value");
            if (countValue && countValue->Type() == util::JsonValueType::string)
            {
                util::JsonString* countString = static_cast<util::JsonString*>(countValue);
                std::string countStr = util::ToUtf8(countString->Value());
...
std::unique_ptr<util::JsonValue> Container::Range(const std::string& containerVarExpr, int64_t rangeStart, int64_t rangeEnd)
{
    std::unique_ptr<util::JsonObject> rangeObject(new util::JsonObject());
    std::unique_ptr<util::JsonArray> rangeArray(new util::JsonArray());
    int64_t count = Count(containerVarExpr);
    if (count != 0)
    {
        std::unique_ptr<util::JsonValue> start = Subscript(containerVarExpr, rangeStart);
        ...
        {
            std::unique_ptr<util::JsonValue> value = iterator->Current();
            rangeArray->AddItem(std::move(value));
            iterator = iterator->Next();
        }
    }
    rangeObject->AddField(U"range", std::move(rangeArray));
    rangeObject->AddField(U"success", std::unique_ptr<util::JsonValue>(new util::JsonBool(true)));
    return rangeObject;
}
...
std::unique_ptr<util::JsonValue> ForwardContainer::Subscript(const std::string& containerVarExpr, int64_t index)
{
    if (index < 0)
    {
...
std::unique_ptr<util::JsonValue> List::Subscript(const std::string& containerVarExpr, int64_t index)
{
    if (index < 0)
    {
        throw std::runtime_error("invalid subscript expression: " + containerVarExpr + "[" + std::to_string(index) + "]");
...
std::unique_ptr<util::JsonValue> ListRepContainer::Subscript(const std::string& containerVarExpr, int64_t index)
{
