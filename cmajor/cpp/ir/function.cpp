...
void Function::Write(util::CodeFormatter& formatter, Context& context, util::BinaryStreamWriter& writer, int32_t& numFunctions)
{
    ...
    if (fileIndex != -1 && !functionId.is_nil())
    {
        ...
        numInstsPos = writer.GetStream().Tell();
    ...
    if (fileIndex != -1 && !functionId.is_nil())
    {
        int64_t currentPos = writer.GetStream().Tell();
        writer.GetStream().Seek(numInstsPos, util::Origin::seekSet);
        cmajor::debug::WriteNumberOfInstructionRecords(writer, numInsts); 
        writer.GetStream().Seek(currentPos, util::Origin::seekSet);
...
void Function::WriteValueDeclarations(util::CodeFormatter& formatter, Context& context)
{
...
void Function::SetSourceModuleId(const util::uuid& sourceModuleId_)
{
    sourceModuleId = sourceModuleId_;
}

void Function::SetFunctionId(const util::uuid& functionId_)
{
