...
module cmajor.systemx.object.text.format;

import cmajor.systemx.object.binary.file;
import cmajor.systemx.object.binary.file.formatter;
...
{
public:
    TextFormatter(cmajor::systemx::machine::Machine& machine, BinaryFile* file_);
...
TextFormatter::TextFormatter(cmajor::systemx::machine::Machine& machine, BinaryFile* file_) :
...
void TextFormatter::FormatSection(CodeSection* section)
{
    if (section && section->Length() > 0)
    {
        ...
        formatter.WriteLine();
        formatter.WriteLine("BASE ADDRESS=#" + util::ToHexString(static_cast<uint64_t>(section->BaseAddress())));
        formatter.WriteLine();
...
{
    formatter.Write("#" + util::ToHexString(currentAddress) + " ");
}

void TextFormatter::FormatAssembledBytes(uint8_t opc, uint8_t x, uint8_t y, uint8_t z)
{
    formatter.Write(util::ToHexString(opc) + " " + util::ToHexString(x) + " " + util::ToHexString(y) + " " + util::ToHexString(z) + " ");
}
...
void TextFormatter::FormatLabel(uint64_t currentAddress)
{
    ...
    if (symbol)
    {
        ...
        {
            formatter.Write(util::Format(symbol->LocalName(), 7, util::FormatWidth::min) + " ");
        }
        else
        {
            formatter.Write(util::Format(symbol->FullName(), 7, util::FormatWidth::min) + " ");
        }
    }
    else
    {
...
{
    formatter.Write(util::Format(opCodeName, cmajor::systemx::machine::MaxOpCodeNameLength(), util::FormatWidth::min) + " ");
}

void TextFormatter::FormatByteOperand(uint8_t operand)
{
    formatter.Write("#" + util::ToHexString(operand));
}
...
{
    formatter.Write("#" + util::ToHexString(wyde));
}

void TextFormatter::FormatAddress(uint64_t address)
{
    formatter.Write("#" + util::ToHexString(address));
...
{
    formatter.Write("#" + util::ToHexString(octa));
