...
module cmajor.systemx.intermediate.writer;
...
namespace {
    ...
    void WriteTypeRef(Type* type, util::CodeFormatter& formatter, int w)
    {
        std::string s;
    ...
    void Write(Value* value, util::CodeFormatter& formatter, int w)
    {
    ...
    void WriteLabel(int32_t id, util::CodeFormatter& formatter, int w)
    {
        formatter.Write(Format("@" + std::to_string(id), w, util::FormatWidth::min));
    }

} // namespace

Writer::Writer(Context* context_, const std::string& fileName) :
    Visitor(context_), file(fileName), fileFormatter(new util::CodeFormatter(file)), formatter(*fileFormatter), newLines(false)
{
    formatter.SetIndentSize(8);
}

Writer::Writer(Context* context_, util::CodeFormatter& formatter_) : Visitor(context_), formatter(formatter_), newLines(true)
{
}
...
void Writer::Visit(BasicBlock& basicBlock)
{
    ...
    formatter.DecIndent();
    formatter.Write(util::Format("@" + basicBlock.Name(), 8));
