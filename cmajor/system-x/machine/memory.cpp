...
module cmajor.systemx.machine.memory;
...
MemoryError::MemoryError(const std::string& message_) : std::runtime_error(message_)
{
}
...
MemoryTranslationMap::MemoryTranslationMap() : flags(MemoryTranslationFlags::none)
{
}
...
void MemoryPage::DecUse()
{
    ...
    if (useCount == 0)
    {
        ...
        if (!retval)
        {
            throw MemoryError("free page failed");
        }
        address = 0;
    }
}
...
Memory::Memory(Machine& machine_) : machine(machine_), maxProcs(MaxProcs()), nextRV(0)
{
    ...
    {
        throw MemoryError("invalid 'maxProcs' (" + std::to_string(maxProcs) + ") in '" + ConfigFilePath() + "': value should be in range 1..." + std::to_string(64 * 1024));
    }
...
uint64_t Memory::AllocateTranslationMap()
{
    ...
    if (nextRV >= maxProcs)
    {
        throw MemoryError("all memory translation maps in use");
    }
    uint64_t rv = nextRV++;
    translationMaps[rv].reset(new MemoryTranslationMap());
    return rv;
}
...
void Memory::SetTextSegmentReadOnly(uint64_t rv, bool readOnly)
{
    auto& translationMapPtr = translationMaps[rv];
    if (!translationMapPtr)
    {
        throw MemoryError("translation map for virtual translation register (rV) value #" + util::ToHexString(rv) + " not allocated");
    }
    auto& translationMap = *translationMapPtr;
    if (readOnly)
    {
        translationMap.flags = translationMap.flags | MemoryTranslationFlags::textSegmentProtected;
    }
    else
    {
        translationMap.flags = translationMap.flags & ~MemoryTranslationFlags::textSegmentProtected;
...
void Memory::FreeMemory(uint64_t rv)
{
    ...
    if (rv >= maxProcs)
    {
        throw MemoryError("invalid virtual translation register (rV) value #" + util::ToHexString(rv) + ": value greater than or equal to maxProcs (#" +
            util::ToHexString(static_cast<uint32_t>(maxProcs)) + ")");
    }
    auto& translationMapPtr = translationMaps[rv];
    if (translationMapPtr)
    {
        auto& translationMap = *translationMapPtr;
        for (auto& p : translationMap.pageMap)
        {
...
void Memory::AllocateRange(uint64_t rv, uint64_t start, uint64_t length)
{
    ...
    for (int64_t i = 0; i < n; ++i)
    {
        ...
        auto it = translationMap.pageMap.find(pageNumber);
        if (it == translationMap.pageMap.cend())
        {
            MemoryPage* page = AllocatePage();
            translationMap.pageMap[pageNumber] = page;
        }
    }
}
...
void Memory::FreeRange(uint64_t rv, uint64_t start, uint64_t length)
{
    ...
    for (int64_t i = 0; i < n; ++i)
    {
        ...
        if (it != translationMap.pageMap.cend())
        {
            ...
            FreePage(page);
            translationMap.pageMap[pageNumber] = nullptr;
        }
    }
}
...
void Memory::CopyRange(uint64_t fromRV, uint64_t toRV, uint64_t start, uint64_t length)
{
    ...
    {
        throw MemoryError("translation map for virtual translation register (rV) value #" + util::ToHexString(fromRV) + " not allocated");
    }
    auto& fromTranslationMap = *fromTranslationMapPtr;
    auto& toTranslationMapPtr = translationMaps[toRV];
    if (!toTranslationMapPtr)
    {
        throw MemoryError("translation map for virtual translation register (rV) value #" + util::ToHexString(toRV) + " not allocated");
    }
    ...
    {
        int64_t pageNumber = s + i;
        auto it = fromTranslationMap.pageMap.find(pageNumber);
        if (it != fromTranslationMap.pageMap.cend())
        {
            MemoryPage* from = it->second;
            MemoryPage* page = CopyPage(from);
            toTranslationMap.pageMap[pageNumber] = page;
        }
        else
        {
            throw MemoryError("page #" + util::ToHexString(static_cast<uint64_t>(i)) + " not found from memory translation map #" + util::ToHexString(fromRV));
        }
    }
}
...
void Memory::Copy(uint64_t rv, uint64_t sourceVirtualAddress, uint64_t targetVirtualAddress, uint64_t count)
{
    ...
    uint64_t from = TranslateAddress(rv, sourceVirtualAddress, cmajor::systemx::machine::Protection::read, fromPageOffset);
    int64_t toPageOffset = 0;
    uint64_t to = TranslateAddress(rv, targetVirtualAddress, cmajor::systemx::machine::Protection::write, toPageOffset);
    while (count > 0)
    {
        ...
        fromPageOffset = 0;
        from = TranslateAddress(rv, sourceVirtualAddress, cmajor::systemx::machine::Protection::read, fromPageOffset);
        toPageOffset = 0;
        to = TranslateAddress(rv, targetVirtualAddress, cmajor::systemx::machine::Protection::write, toPageOffset);
    }
}
...
uint64_t Memory::TranslateAddress(uint64_t rv, uint64_t virtualAddress, Protection access, int64_t& pageOffset)
{
    ...
    auto& translationMap = *translationMapPtr;
    if ((access & Protection::write) != Protection::notPresent && virtualAddress >= textSegmentBaseAddress && virtualAddress < dataSegmentBaseAddress)
    {
        CheckWriteToTextSegment(translationMap, rv, virtualAddress);
    }
    ...
    {
        throw MemoryError("virtual address #" + util::ToHexString(virtualAddress) + " not found from memory translation map #" + util::ToHexString(rv));
    }
}

void Memory::CheckWriteToTextSegment(MemoryTranslationMap& translationMap, uint64_t rv, uint64_t virtualAddress)
{
    if ((translationMap.flags & MemoryTranslationFlags::textSegmentProtected) != MemoryTranslationFlags::none)
    {
        throw MemoryError("access violation: tried to write to read-only text segment: "
            "virtual translation register (rV) value=#" + util::ToHexString(rv) + ", virtual address=#" + util::ToHexString(virtualAddress));
    }
}
