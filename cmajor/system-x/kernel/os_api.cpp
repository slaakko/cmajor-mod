...
module cmajor.systemx.kernel.os.api;
...
std::string OsKeyName(int key)
{
    switch (key)
    {
        ...
        default:
        {
            ...
            {
                return "<key-" + util::ToUtf8(std::u32string(1, key)) + ">";
            }
            return "<unknown-key>";
        }
    }
...
{
    std::u16string s = util::ToUtf16(util::MakeNativePath(directoryName) + "\\*.*");
...
void OsWriteConsole(void* consoleOutputHandle, const char32_t* chars)
{
    ...
    std::u16string w(util::ToUtf16(s));
...
util::DateTime FileTimeToDateTime(FILETIME* fileTime)
{
    SYSTEMTIME utc;
    SYSTEMTIME local;
    bool retval = FileTimeToSystemTime(fileTime, &utc);
    if (!retval)
    {
        ThrowLastHostError(std::string());
    }
    retval = SystemTimeToTzSpecificLocalTime(nullptr, &utc, &local);
    if (!retval)
    {
        ThrowLastHostError(std::string());
    }
    util::Date date(local.wYear, static_cast<util::Month>(local.wMonth), local.wDay);
    int32_t secs = static_cast<int32_t>(60) * 60 * local.wHour + 60 * local.wMinute + local.wSecond;
    return util::DateTime(date, secs);
}
...
void OsGetFileTimes(const char* filePath, util::DateTime& accessTime, util::DateTime& modifiedTime, util::DateTime& createTime)
{
    ...
    FILETIME mtime;
    FILETIME ctime;
    void* fileHandle = OsOpenHostFile(filePath, false);
    bool retval = GetFileTime(fileHandle, &ctime, &atime, &mtime);
    if (!retval)
    {
        ThrowLastHostError(std::string());
    }
    accessTime = FileTimeToDateTime(&atime);
    modifiedTime = FileTimeToDateTime(&mtime);
    createTime = FileTimeToDateTime(&ctime);
