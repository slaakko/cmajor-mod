...
module cmajor.systemx.kernel.fs;
...
{
    util::MemoryReader reader(block->Data(), Block::Size());
...
{
    util::MemoryWriter writer(block->Data(), Block::Size());
...
{
    std::string parentDirectoryPath = util::Path::GetDirectoryName(path);
    ...
    WriteINode(dirINode, process);
    DirectoryEntry entry;
    entry.SetName(util::Path::GetFileName(path));
...
INodePtr PathToINode(const std::string& path, Filesystem* fs, cmajor::systemx::machine::Process* process, PathToINodeFlags flags)
{
    ...
    for (int i = start; i < components.size(); ++i)
    {
        ...
        dirPath = util::Path::Combine(dirPath, name);
    }
