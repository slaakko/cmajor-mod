...
module cmajor.systemx.kernel.terminal;
...
private:
    ...
    cmajor::systemx::machine::Event terminalInputEvent;
    ...
    bool started;
    bool stopped;
    util::Utf8ToUtf32Engine writeEngine;
    util::Utf8ToUtf32Engine readEngine;
    ...
    if (mode == TerminalMode::raw)
    {
        ...
        else
        {
            ...
            for (char c : chars)
            {
                ...
                if ((GetDebugMode() & debugTerminalMode) != 0)
                {
                    DebugWrite("kernel.terminal: handle.input.char(" + util::ToHexString(static_cast<uint8_t>(c)) + ")");
                }
            }
        }
        if ((GetDebugMode() & debugTerminalMode) != 0)
...
void Terminal::GetCompletions(std::unique_lock<std::recursive_mutex>& lock)
{
    ...
    try
    {
        cmajor::systemx::kernel::Process* process = ProcessManager::Instance().GetProcess(pid);
        if (process)
        {
            ...
            util::MemoryWriter writer(putTabMsg.data(), count);
            ...
            if (success)
            {
                ...
                for (int32_t i = 0; i < numCompletions; ++i)
                {
                    ...
                    completions.push_back(Completion(util::ToUtf32(ln), pos));
                }
...
std::vector<uint8_t> Terminal::Read(int64_t count, cmajor::systemx::machine::Process* process)
{
...
int64_t Terminal::Write(const std::vector<uint8_t>& buffer, cmajor::systemx::machine::Process* process)
{
...
void Terminal::PutKeyPressedMessage(char32_t ch)
{
    ...
    util::MemoryWriter writer(keyPressedMessageData.data(), 8 + 4);
