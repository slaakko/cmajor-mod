...
module cmajor.systemx.kernel.file;
...
std::vector<uint8_t> File::Read(int64_t count, cmajor::systemx::machine::Process* process)
{
    if (!IsReadable())
    {
...
int64_t File::Write(const std::vector<uint8_t>& buffer, cmajor::systemx::machine::Process* process)
{
    if (!IsWritable())
    {
...
int64_t File::Seek(int64_t offset, Origin whence, cmajor::systemx::machine::Process* process)
{
    throw SystemError(EBADF, name + " not seekable", __FUNCTION__);
}

int64_t File::Tell(cmajor::systemx::machine::Process* process)
{
    throw SystemError(EBADF, name + " not seekable", __FUNCTION__);
}
...
{
    files.resize(cmajor::systemx::machine::MaxOpenFiles());
    files[0] = terminalFile;
    files[1] = terminalFile;
    files[2] = terminalFile;
}

void ProcessFileTable::CloseFiles(cmajor::systemx::kernel::Process* process)
{
    for (File* file : files)
    {
        if (file)
...
void ProcessFileTable::CloseFile(int32_t fd, cmajor::systemx::kernel::Process* process)
{
    File* file = GetFile(fd);
    file->Close(process);
    files[fd] = nullptr;
...
{
    cmajor::systemx::kernel::terminalFile = terminalFile;
}

File* GetTerminalFile()
{
    return cmajor::systemx::kernel::terminalFile;
}

void WriteToTerminal(const std::string& text, cmajor::systemx::machine::Process* process)
{
    if (cmajor::systemx::kernel::terminalFile)
    {
        ...
        cmajor::systemx::kernel::terminalFile->Write(buffer, process);
    }
}
