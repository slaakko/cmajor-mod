...
export module cmajor.token.value.parser;
...
export namespace cmajor::token::value::parser {
...
template<typename Char>
char32_t ParseCharacterLiteral(const std::string& fileName, const soul::lexer::Token<Char, soul::lexer::LexerBase<Char>>& token, cmajor::ast::CharLiteralPrefix& prefix)
{
    prefix = cmajor::ast::CharLiteralPrefix::none;
    char32_t value = '\0';
    const Char* p = token.match.begin;
    const Char* e = token.match.end;
    if (p != e && *p == 'w')
    {
        prefix = cmajor::ast::CharLiteralPrefix::utf16Prefix;
        ++p;
    }
    else if (p != e && *p == 'u')
    {
        prefix = cmajor::ast::CharLiteralPrefix::utf32Prefix;
        ++p;
    }
    if (p != e && *p == '\'')
    {
...
} // namespace cmajor::token::value::parser
