...
void LLvmEmitter::Compile(const std::string& objectFilePath)
{
    ...
    ::llvm::legacy::FunctionPassManager functionPassManager(impl->module);

    int optLevel = EmittingContext()->OptimizationLevel();
    int sizeLevel = 0;
    if (optLevel > 0)
    {
    ...
    if (optLevel > 0)
    {
        ...
        for (::llvm::Function& f : *impl->module)
        {
